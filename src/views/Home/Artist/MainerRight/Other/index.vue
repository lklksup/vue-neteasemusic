    ...
    <div class="Other">
        <div class="all" v-show="area == -1 && type == -1">
            <div class="rec clearfix">
                ...
                <!-- 内容 -->
                <div class="singer_list clearfix">
                    <li class="singer" v-for="singer, index in all_artist_list" :key='index'>
                        <div class="cover">
        ...
        <div class="oth clearfix" v-show="area != -1">
            ...
            <!-- 字母索引条 -->
            <ul class="n-list">
                <li class="underline" v-for="item, index in nlist" :key="index" @click="changeInitial(item)"
            ...
            <div class="singer_list clearfix">
                <li class="singer" v-for="singer, index in artist1" :key='index'>
                    ...
                    <p>
                        <router-link :to="{ path: '/artist/song', query: { id: singer.id } }" class="nm underline">
            ...
            <div class="moreOther clearfix">
                <li class="sml" v-for="sml, index in artist2" :key="index">
                    <router-link :to="{ path: '/artist/song', query: { id: sml.id } }" class="over underline">{{ sml.name }}
                    </router-link> <i class="pointer" v-if="sml.alias.length != 0"></i>
                </li>
            </div>
        </div>
    </div>
...
export default {
    ...
    methods: {
        ...
        async getArtistTop(limit) {
            ...
            this.all_artist_list = result.artists;
        },
        //获取分类歌手
        async getArtistsList() {
            ...
            this.artist2 = result.artists.slice(10);
        },
        //改变initial值
        changeInitial(item) {
            if (item == '热门') item = -1;
