...
export default {
    ...
    methods:{
        // 获取歌手专辑
        async getArt(){
            let result = await this.$API.reqArtistAlbum(this.uid,5,1,localStorage.getItem('COOKIE')); 
            console.log(result);   
        ...
        // 获取专辑描述
        async getAlbum(){
            let result = await this.$API.reqAlbums(this.id,localStorage.getItem('COOKIE'));
