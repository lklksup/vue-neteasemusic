...
const actions={
    ...
    async getMusicLists({commit}){
        ...
        let result = await reqSong(list,localStorage.getItem('COOKIE'));
    ...
    async setMusics({commit},id){
        setMusic(id);
        let result = await reqSong(id,localStorage.getItem('COOKIE'));
        commit('SETMUSIC',result.songs[0]);
    },
    // 获取当前播放的音乐
    async getMusics({commit}){
        let result = await reqSong(getMusic(),localStorage.getItem('COOKIE'));
        commit('SETMUSIC',result.songs[0]);
    }
}
const getters={
